...
**基础信息区**（顶部或左侧）：
   ...
   - 行政职务（administrative_position）：**谨慎提取，不要混淆职称和职务**
  **职务与职称的区别：**
  - **职称**：主任医师、副主任医师、主治医师、主任技师、副主任技师等（专业技术级别）
  - **职务**：科室主任、副主任、院长、副院长、护士长等（行政管理职位）
  **提取规则：**
  - 只有明确提到"科室主任"、"科主任"、"副主任"、"院长"等才提取为职务
  - **不要把"林琳副主任"提取为职务！** 这应该是"姓名：林琳"+"职称：副主任技师"
  - **姓名后面紧跟的"副主任"通常是职称的一部分，不是职务**
  - 如果只有"副主任技师"、"副主任医师"等，这是职称，职务应为空/null
  - 职务字段在没有明确信息时应设为 null，不要强行提取
   - 医生照片
...
- 擅长（specialty）：**必须从多个位置查找**
  **提取优先级：**
  ...
  3. **简介文本中的擅长关键词**
     - 关键词：擅长、专长、主治、善于诊治
     - 例如："擅长急性白血病、慢性白血病及骨髓瘤的化疗..."
  **提取规则：**
  - 必须提取完整的擅长内容，不能截断
  - 如果有多个段落，必须全部提取并合并
  - **如果页面有"专业特长"区域但没有提取到内容，说明遗漏了！**
