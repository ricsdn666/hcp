...
**页面区域遍历顺序（必须全部检查）：**
1. 顶部基本信息区（姓名、职称、科室、行政职务等）
2. 键值对信息区（学历、职称、专业方向等）
3. **详细介绍/个人简介区**（通常在"详细介绍"、"个人简介"等标题下）
4. **专业特长/擅长区**（通常在"专业特长"、"擅长"、"擅长领域"等标题下）
5. 社会职务区
6. 主要成果区
7. 业务经历区（**重要：包含教育背景！**）
8. 其他区域
...
**重要提示：**
- 很多医院网站没有标准键值对布局，信息分布在段落文本中
- 必须从段落文本中提取结构化信息
- **"详细介绍"、"个人简介"标题下的内容 = 简介（intro）**
- **"专业特长"、"擅长"、"擅长领域"标题下的内容 = 擅长（specialty）**
...
**基础信息区**（顶部或左侧）：
   - 医生姓名（h1/h2标题，或照片旁边的文本）
   - **行政职务和职称可能在同一行**，需要分离：
     - 例如："科主任 主任医师" → administrative_position="科主任", title="主任医师"
     - 例如："副院长 主任医师" → administrative_position="副院长", title="主任医师"
     - **规则：** 姓名下方的文本，如果有空格分隔，第一个是职务，第二个是职称
   - 职称（标签或介绍文本，也可能在键值对中）
...
**个人简介区**（通常在"详细介绍"、"个人简介"、"医生简介"等标题下）：
   - 医生简介原文（intro）：必须完整提取
   - **学历信息通常在这里**：
     - 例如："1990 年河南医科大学本科毕业，2001 年郑州大学硕士研究生毕业"
     - 提取：education="硕士"（最高学历），alma_mater="郑州大学"
   - **工作年限通常在这里**：
     - 例如："从事呼吸内科临床工作 30 余年"
   ...
   - 学术任职
   - 科研成果
   - **注意：如果页面只有照片没有文本信息，说明信息可能在图片中**
   - **如果信息在图片中，需要尝试识别图片中的文字（OCR）**
   - **常见图片信息格式**：
     - 照片下方或旁边的文字说明
     - 图片中嵌入的文字信息
     - 例如：照片旁标注"毕业于菏泽医专"、"本科学历"等
...
- 科室：展示科室名
  **从以下位置提取（按优先级）：**
  1. **键值对中的"开诊科室"、"科室"、"所属科室"、"科室/病区"**（最优先）
  2. **页面顶部的科室信息**（通常在姓名下方或旁边）
     - 例如："刘延方血液内科" → 科室="血液内科"
  3. **页面上的科室链接或文本**
     - 例如：导航路径中的"血液病医学部" > "血液内科"
     - 例如：面包屑导航显示的科室名称
  **注意：**
  - 如果姓名和科室连在一起（如"刘延方血液内科"），必须分离
  - 优先提取具体的临床科室（如"血液内科"），而不是学部（如"血液病医学部"）
  - **绝对不能从 URL 中提取科室名！** URL 中的路径不是科室名
  - **科室名必须是完整的中文名称**，不能是拼音缩写（如"pfk"）
  - **如果找不到明确的科室信息，设为 null**，不要强行提取

- 学历（education）：**必须从多个位置查找**
  **提取优先级：**
  1. **键值对中的"学历"**（最优先！必须检查！）
     - 常见格式："学历：本科"、"学历：硕士"、"学历：博士"
     - **如果页面有键值对区域，必须检查是否有"学历"键**
  2. **个人简介中的学历关键词**（常见！）
     - 直接学历词：博士、硕士、本科、研究生、大专
     - 学历组合词：研究生学历、本科学历、博士学历、硕士学历、大专学历
     - **毕业格式**："XX 年 XX 大学本科毕业"、"XX 年 XX 大学硕士研究生毕业"
     - **学历 + 毕业格式**："毕业于 XX 大学，本科学历"、"毕业于 XX 大学，硕士学历"
     - 例如："1990 年河南医科大学本科毕业，2001 年郑州大学硕士研究生毕业"
       - 提取：education="硕士"（最高学历）
     - 例如："毕业于山东中医药大学，本科学历" → education="本科", alma_mater="山东中医药大学"
  ...
  **核验：**如果页面有键值对但没有提取到学历，说明可能遗漏了！

- 工作年限（years_of_experience）：**必须从多个位置查找**
  **提取优先级：**
  1. **键值对中的"工作年限"、"从业年限"**（最优先）
  2. **个人简介文本中的年限关键词**（常见！）
     - 模式：`从事...工作 XX 年 `、` 从业 XX 年 `、` 临床工作 XX 年`、` 参加工作 XX 年`、` 从医 XX 年`、` 从事本专业工作 XX 年`、` 从事...工作已 XX 余年`
     - 例如：`从事神经外科近 20 年` → years_of_experience=20
     - 例如：`从事呼吸内科临床工作 30 余年` → years_of_experience=30
     - 例如：`自参加工作 30 年以来` → years_of_experience=30
     - 例如：`从医 20 余年` → years_of_experience=20
     - 例如：`从业 20 余年` → years_of_experience=20
     - 例如：`从事本专业工作 20 余年` → years_of_experience=20
     - 例如：`从事医学影像诊断及放射介入治疗医疗工作已 40 余年` → years_of_experience=40
     - 关键词：从事、从业、工作、余年、近、余、参加工作、从医、本专业、已
  3. **业务经历中的时间推算**
     - 例如：`1984-1997工作` → 可推算约 13 年
  **重要排除规则：**
  - **规培年限 ≠ 工作年限！** 例如："规范化培训 3 年"不能提取为工作年限
  - **进修年限 ≠ 工作年限！** 例如："进修半年"不能提取为工作年限
  - **学习/培训年限 ≠ 工作年限！**
  - 只有明确提到"从事...工作 XX 年"、"临床工作 XX 年"或"从医 XX 年"才能提取
  **提取规则：**
  - 提取数字部分，忽略"近"、"余"、"以来"、"已"等修饰词
  - 如果有多个工作年限描述，取最近/最准确的
  - 如果找不到，设为 null，不要设为 0

**擅长（specialty）：**必须从多个位置查找
  **提取优先级：**
  1. **键值对中的"擅长"、"专业特长"、"诊疗特长"**（最优先）
  2. **"专业特长"、"擅长"、"擅长领域"等标题下的内容区域**
     - 找到"专业特长"、"擅长"等标题
     - 提取该标题下方的所有文本内容，直到下一个标题开始
  3. **简介文本中的擅长关键词**
     - 关键词：擅长、专长、主治、善于诊治
     - 例如："擅长肺癌的诊断及治疗，肺穿刺活检阳性率高..."
  **提取规则：**
  - 必须提取完整的擅长内容，不能截断
  - 如果有多个段落，必须全部提取并合并
  - **如果页面有"专业特长"、"擅长"等标题但没有提取到内容，说明遗漏了！**

**行政职务（administrative_position）提取规则：**
- **位置 1：表格/键值对中的"职务"、"职位"信息**（最优先）
  - 例如：表格中 `职务 | 主任` → administrative_position="主任"
  - 例如：表格中 `职位 | 副主任` → administrative_position="副主任"
  - 例如：表格中 `职务 | 中医科主任` → administrative_position="中医科主任"
  - 例如：表格中 `职务 | 副主任` → administrative_position="副主任"
  - 例如：`职务：科主任` → administrative_position="科主任"
  - **必须检查表格中的"职务"列/单元格！也要检查"职位"列！**
  - **常见职务值：主任、副主任、科主任、科室主任等**
  - **键名可能是"职务"或"职位"，都要检查！**
- **位置 2：姓名下方/旁边的文本**
  - 例如："科主任 主任医师" → administrative_position="科主任"
  - 例如："副院长 主任医师" → administrative_position="副院长"
  - **规则：** 姓名下方的文本，如果有空格分隔，第一个是职务，第二个是职称
- **位置 3：简介文本中的职务信息**
  - 例如："呼吸与危重症医学科三病区主任" → administrative_position="科主任"
  - 例如："皮肤科副主任" → administrative_position="副主任"
  - 例如："中医科主任" → administrative_position="中医科主任"
  - 例如："放疗科筹建人 放疗科主任" → administrative_position="主任"
- **位置 4：图片中的文字信息**（如果页面只有照片没有文本）
  - 检查照片下方、旁边的文字说明
  - 尝试识别图片中嵌入的文字信息
- **常见职务：** 科主任、科室主任、副主任、院长、副院长、护士长、主任、副主任
- **注意：** 职务通常是行政管理职位，不是技术职称
...
- 职称（title）：**必须准确提取，特别注意区分正高和副高**
  **提取优先级：**
  1. **键值对中的"职称"**（最优先，最准确！）
     ...
     - 例如："职称：主任中医师" → title="主任中医师"
     - 例如："职称：副主任中医师" → title="副主任中医师"
  2. **个人简介中的职称关键词**
     - 在简介开头通常有职称信息
     - 例如："张梦真，女，教授，博士，主任医师" → title="主任医师"
     - 例如："张朝晖，主任中医师，南京医科大学副教授" → title="主任中医师"
     - 关键词：主任医师、副主任医师、主治医师、教授、副教授、研究员、**主任中医师、副主任中医师、主治中医师**等
     - **中医类职称：**主任中医师、副主任中医师、主治中医师、中医医师（注意带"中"字）
  ...
  **关键区分：**
     ...
     - **"主任医师" → title="主任医师"**
     - **"主治中医师" → title="主治中医师"**
     - **"主任中医师" → title="主任中医师"（中医类正高职称）**
     - **"副主任中医师" → title="副主任中医师"（中医类副高职称）**
     - **"主治医师" → title="主治医师"**
     - **"副主任技师" → title="副主任技师"（注意是"技师"不是"医师"）**
  **提取规则：**
     - **必须完整提取职称文本，不能省略"副"字、"中"字！**
     - **如果键值对中明确写"副主任医师"或"主任中医师"，必须原样提取！**
     - **这是最常见的错误，务必注意！**
     - 如果键值对中写"职称/职务：省内知名专家"等非标准职称：
       - 先查找简介中是否有明确职称
       - 如果没有，查看社会职务级别推断
       - 如果仍无法确定，设为 null，不要强行提取
     - 常见职称全称：主任医师、副主任医师、主治医师、住院医师、医师、主任护师、副主任护师、主管护师、护师、护士、教授、副教授、研究员、副研究员、主任技师、副主任技师、主管技师、技师、**主任中医师、副主任中医师、主治中医师、中医医师**

- 简介：完整的个人介绍原文
  **重要：简介通常在以下位置：**
  - 页面中标注"个人简介"、"医生简介"、"人物介绍"等**标题下的内容区域**
  - 包含教育背景、工作经历、学术成就等段落
  - **必须提取完整内容，不能遗漏！**
  **提取规则：**
  - 找到"个人简介"、"医生简介"等标题
...
- 毕业院校（alma_mater）：**必须从多个位置查找**
  **提取优先级：**
  ...
  2. **业务经历/工作经历区域的文本**（常见）
     - 模式：年份 + 院校名 + 专业 + "毕业"
     - 例如："1988 年河南医科大学临床医学系毕业后" → alma_mater="河南医科大学"
     - 例如："毕业于郑州大学医学院" → alma_mater="郑州大学"
  3. **个人简介中的教育背景描述**
     - 关键词：毕业、院校、大学、学院、就读、毕业于
  4. **图片中的文字信息**（如果页面只有照片没有文本）
     - 检查照片下方、旁边的文字说明
     - 尝试识别图片中嵌入的文字信息
     - 例如：照片旁标注"毕业于菏泽医专"、"菏泽医学专科学校"等
  **提取规则：**
  - 提取完整院校名称，不要专业名
  - "河南医科大学" ✓ "河南医科大学临床医学系" ✗
  - 多个院校时，取最高学历对应的院校
  - **专科/高职类院校也要提取**：如"菏泽医专"、"菏泽医学专科学校"、"xx 卫校"等
...
1. **姓名格式**：2-4个汉字，且不包含以下词汇：
   ...
   - **常见错误模式检测：**
     ...
     - **例如"林琳副主任" 是错误的，姓名="林琳"，职称="副主任技师"**

2. **医院名称**：必须包含目标医院名称

3. **URL 格式**：必须是有效的医生详情页 URL

4. **科室名称**：必须与医院官网的科室名一致
   - 科室名必须是完整的中文名称
   - **不能是拼音缩写（如"pfk"、"nk"等）**
   - **不能从 URL 路径中提取科室名**
   - 如果找不到明确的科室信息，设为 null
...
5. **职称准确性**：必须与页面显示完全一致
   ...
   - **技师类职称也要准确：副主任技师 ≠ 主任技师**

6. **职务准确性**：不要把职称误认为职务
   - **错误示例："林琳副主任"作为职务 → 错误！**
   - 正确提取：姓名="林琳"，职称="副主任技师"，职务=null
   - 只有"科室主任"、"科主任"、"院长"等行政管理职位才算是职务
   - "副主任技师"、"副主任医师"是职称，不是职务

7. **学历完整性**：必须检查所有可能的来源
   - 如果个人简介中有"本科"、"硕士"、"博士"等学历信息，必须提取
   - 例如："副主任技师，本科，2006 年毕业于..." → education="本科"
   - **如果页面有明确的学历信息但没有提取到，说明遗漏了！**
...
<div class="doctor-info">
    ...
    <div class="info-item">
        ...
        <span class="value">从事临床工作 30 余年</span>
    </div>
    <div class="info-item">
...
**表格提取规则：**
- 识别表格中的键值对：相邻的两个单元格，第一个是键名，第二个是值
- 常见键名：姓名、职称、职务、科室、擅长领域、个人简介等
- **必须仔细检查表格中的每一行，不要遗漏任何信息！**
