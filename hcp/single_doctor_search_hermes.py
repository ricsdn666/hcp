...
**⚠️ 重要限制（严格执行）：**
1. **可以使用任何搜索引擎和直接访问医疗平台**
2. **搜索次数不得超过5次（速度优先）**
3. **优先选择最快的方式获取数据：直接访问已知医疗平台 > 搜索引擎搜索**
4. **效率优先：快速抓取、快速推送**
5. **URL 必须验证：填写前访问 URL 确认页面包含医生，验证失败则不推送**
...
1. **搜索阶段（速度优先）**
   - **可以使用任何搜索引擎：百度、Google、必应、搜狗、360搜索等**
   - **可以直接访问医疗平台：好大夫、百度健康、医院官网、有来医生等**
   - **优先级排序（从高到低）**：
     1. 医院官网（最权威、数据最准确）
     2. 好大夫在线（专业医疗平台）
     3. 百度健康（权威医疗信息平台）
     4. 有来医生、妙手医生等
     5. **名医汇（优先级最低，数据质量较差，仅在找不到其他来源时使用）**
   - **优先选择最快的方式**：
     1. 如果已知医生所在医院官网，直接访问更快
     2. 如果有常用医疗平台链接，直接访问更快
     3. 使用搜索引擎作为备选方案
   - 搜索关键词："{__name__} {__hospital__} {__dept__}" 或 "{__name__} {__hospital__}"
   - **最多搜索/访问5次，不超过5次**
   - **跳过名医汇结果，优先点击医院官网、好大夫、百度健康等权威医疗网站的链接**
   - **速度优先：找到第一个有效链接即可，不要反复比较**
...
3. **输出阶段**
   ...
   - **URL 处理规则（严格执行）**：
     ...
     3. **验证成功**：页面包含目标医生 → otherpropertys.url 填写该 URL，推送数据
     4. **验证失败**：页面不包含目标医生 → 该 URL 无效，不推送该医生数据
     5. **没有找到 URL**：不推送该医生数据
   - **必须找到有效 URL 才推送**
...
| **source_model**        | **string**     | **数据来源模型（固定值）**       | **必须固定为 "Hermes"，不能修改为其他任何值**                                                                                                                                               |
...
1. **数据真实性**
   - 必须基于实际搜索/访问结果填写，不可编造
   - 未找到的字段填 `null`
   - 可以使用任何搜索引擎或直接访问医疗平台
    
2. **搜索限制（速度优先）**
   - **可以使用任何搜索引擎：百度、Google、必应、搜狗、360等**
   - **可以直接访问医疗平台：好大夫、百度健康、医院官网等**
   - **搜索/访问次数不得超过5次**
   - **优先选择最快的方式：直接访问已知平台 > 搜索引擎搜索**
   - **平台优先级（从高到低）**：
     1. 医院官网（最权威）
     2. 好大夫在线（专业医疗平台）
     3. 百度健康（权威医疗信息）
     4. 其他医疗平台
     5. **名医汇（优先级最低，数据质量较差，尽量避免使用）**
   - 如5次搜索/访问后仍未找到有效 URL，不推送该医生数据
...
3. **效率优先原则**
   ...
   - **找到第一个有效链接即可，不要反复比较多个搜索结果**
   - **URL 必须验证有效：访问 URL 确认页面包含目标医生**

4. **特殊情况处理（简化版）**
   - 如医生不在指定医院，直接返回找到的医院信息即可，无需额外标注
   - 如科室不匹配，直接返回找到的科室信息即可，无需额外标注
   - **必须找到有效的医生详情页 URL 才能推送数据**
   - **没有找到医生详情页 URL 或 URL 验证失败，不推送该医生数据**
   - **不要浪费时间处理特殊情况，直接推送数据**
   - **如果5次搜索后仍未找到有效 URL，不推送数据，标记为未找到**
...
- **必须同时有医生姓名和有效 URL，才可以推送数据**
- **URL 处理规则（严格执行）**：
  - **填写 URL 前必须验证：访问 URL，确认页面包含医生**
  - **验证成功**：可以填写该 URL
  - **验证失败**：URL 无效，不推送该医生数据
  - **未找到 URL**：不推送该医生数据
  - **绝不推送无效 URL 或空 URL**
- **没有找到有效 URL 则不推送该医生数据**
- **在 5 次搜索限制内优先找有效 URL，找不到则不推送**
---
...
**⚠️ 重要提醒：**
- `otherpropertys.url` **必须验证且有效，没有有效 URL 则不推送数据**
- **验证流程**：
  1. 找到候选 URL → 2. 访问 URL → 3. 搜索医生姓名 → 4. 验证结果
- **验证成功**：页面包含医生 → 可以填写该 URL，可以推送数据
- **验证失败**：页面不包含医生 → URL 无效，不推送该医生数据
- **未找到 URL**：不推送该医生数据
- **没有有效 URL 则不推送该医生数据**
- **绝不推送无效的 URL 或空 URL**
- **在 5 次搜索限制内优先找有效 URL，找不到则不推送**
- **`source_model` 字段值必须固定为 "Hermes"，绝对不能修改为其他任何值**
...
2. **URL 有效性验证（严格执行）**：
   ...
   - 验证步骤：
     ...
     2. 访问该 URL
     3. 在页面中搜索医生姓名
     4. 页面包含医生姓名 → URL 有效，可以填写并推送数据
     5. 页面不包含医生姓名 → URL 无效，不推送该医生数据
   - **验证失败时不推送，绝不填写无效 URL**
   - **没有找到 URL 时不推送**
   - **在 5 次搜索限制内优先找有效 URL，找不到则不推送**

3. **source_model 固定值**：
   - **source_model 必须为 "Hermes"**
   - 这是唯一必须严格验证的字段

**核验失败的处理：**
- **必须同时有医生姓名和有效 URL 才推送**
- **URL 无效或为空则不推送**
- **绝不推送无效的 URL**
- **其他字段缺失不影响推送，但没有有效 URL 则整体不推送**
- 找到信息立即推送，效率优先
---
...
5. **搜索次数限制**：严格控制在5次以内（速度优先）
6. **遇到任何问题**：不推送，标记为未找到，继续下一个医生
...
4. **搜索限制（速度优先）**：
   ...
   - **可以直接访问医疗平台**
   - **搜索/访问次数不得超过5次**
   - **优先选择最快的方式：直接访问 > 搜索引擎搜索**
   - **避免使用名医汇（优先级最低，数据质量较差）**
   - **优先使用：医院官网 > 好大夫 > 百度健康 > 其他平台 > 名医汇**
...
def safe_ack(channel, delivery_tag, requeue=False):
    ...
    try:
        if requeue:
            ...
            print(f"✅ 消息已 ack 认删除 (delivery_tag={delivery_tag})")
        sys.stdout.flush()
        return True
...
def process_hospital(channel, hospital_data: Dict[str, Any], delivery_tag):
    global hermes_client, processed_count
    ...
    try:
        print("🤖 发送任务到 Hermes Agent...")
        print(f"   API 地址：{hermes_client.config.base_url}")
        print(f"   API Key：{hermes_client.config.api_key}")
        print(f"   超时时间：{hermes_client.config.timeout} 秒")
        print("-" * 70)
        sys.stdout.flush()
        ...
        events = hermes_client.chat(prompt)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Hermes API 地址 (默认：http://127.0.0.1:8642)",
    )
    parser.add_argument(
        "--hermes-key",
        type=str,
        default="123456",
        help="Hermes API Key (默认：123456)",
    )
    args = parser.parse_args()

    hermes_base_url = args.hermes_url
    hermes_api_key = args.hermes_key

    print("\n" + "=" * 70)
    print("🚀 医生信息抓取器启动 (使用 Hermes)")
    print("=" * 70)
    print(f"📍 消费者 ID: {my_consumer_id}")
    print(f"📍 RabbitMQ URL: {RABBITMQ_URL.replace('@', '***@')}")
    print(f"📥 消费队列：{CACHEHOSP_QUEUE}")
    print(f"🤖 Hermes API: {hermes_base_url}")
    print(f"🔑 Hermes API Key: {hermes_api_key}")
    ...
    config = HermesConfig(
        base_url=hermes_base_url, api_key=hermes_api_key, timeout=3600.0
    )
    hermes_client = HermesClient(config)

    print("\n🔍 检查 Hermes API 连接...")
    sys.stdout.flush()

    if not hermes_client.health_check():
        print(f"\n❌ 无法连接到 Hermes API: {hermes_base_url}")
        print("   请检查:")
        print("   1. Hermes 服务是否运行")
        print("   2. API 地址是否正确")
        print("   3. API Key 是否正确")
        print("   4. 网络连接是否正常")
        sys.exit(1)

    print("✅ Hermes API 连接成功\n")
    sys.stdout.flush()
