...
def _fee2patient(year):
    ...
    while(start < end):
        #cur = conn.cursor()
        sql_patient = f"""select czrk_id,outpatient_num from v3_emr_outpatient_record where date(treatment_date)  = date('{start.strftime("%Y-%m-%d")}')"""
        ...
        for r in rs:
            #logger.info(r[0],r[1])
            sql_all_cost = f"""update v3_emr_outpatient_record t1
    set total_amount = (select sum(t2.proj_mon)
                              from v3_emr_out_fee_info_child t2
                              where t2.czrk_id = '{r[0]}'
                                and t2.outpatient_num = '{r[1]}') where t1.czrk_id = '{r[0]}' and outpatient_num = '{r[1]}'
                                """
