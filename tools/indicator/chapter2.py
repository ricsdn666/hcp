...
def init_meta():
    ...
    f= open("./chapter2", mode="r", encoding="utf-8-sig")
    reader = csv.reader(f)
    header = next(reader)
    ...
    for row in reader:
        ...
        if _indicator_id.startswith('0'):
            print(row)
        for y in ys:
            sql = f"INSERT INTO ecrf.v3_indicator_meta_2 (indicator_id, year, name, value, create_time, update_time, unit) VALUES ('{indicator_id}', '{y}', '{name}', '{value}', '{datetime.datetime.now()}', '{datetime.datetime.now()}', '{unit}');"
